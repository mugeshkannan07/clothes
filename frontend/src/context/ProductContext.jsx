...
export const ProductProvider = ({children}) => {
  ...
  useEffect(() => {
    const fetchCart = async () => {
      if (!UserId) return;
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/cart/user/${UserId}`);
 ...
 const addToCart = async (product) => {
  ...
  try {
    const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/cart/add`, newCart);
